...
HirealchemyHcl.controller('MainController', function ($scope, $q, $http) { 
...
HirealchemyHcl.controller('SummaryController', function ($scope, $q, $http) {  
	...
	$scope.name_span = false;
	$scope.name_span_m = true;
	$scope.cap_span = false;
	$scope.cap_span_m = true;
	$scope.dst_span = false;
	$scope.dst_span_m = true;
	$scope.city_span = false;
	$scope.city_span_m = true;
	$scope.population_span = false;
	$scope.population_span_m = true;
	$scope.miles_span = false;
	$scope.miles_span_m = true;
	$scope.time1_span = false;
	$scope.time1_span_m = true;
	$scope.time2_span = false;
	$scope.time2_span_m = true;
	...
	$scope.LoadStatesSort = function(s, offset, hide_id, show_id) {
	    $scope.LoadStates(s, offset);
	    $scope[hide_id] = false;
	    $scope[show_id] = true;
	}
		...
		$scope.prev = 'disabled';
            ...
            if(($scope.offset+$scope.limit)  >= 50) {
		$scope.next = 'disabled';
	    }else {
		$scope.next = '';
            }
            ...
            $scope.answers = {};
            $http({
                url: '/states?sort='+sort+'&offset='+offset+'&limit='+$scope.limit,
                method: 'GET',
                data: params}).success(function(data) {
		//console.log(data, 'data');
                $scope.states = data.res;  
		$scope.total = data.len;  
            }).error(function(data) {
                $scope.loginerror = "Error in server!";
            });
        }
